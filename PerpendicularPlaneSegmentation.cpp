...
#include "PerpendicularPlaneSegmentation.h"


PerpendicularPlaneSegmentation::PerpendicularPlaneSegmentation(void)
{}
PerpendicularPlaneSegmentation::~PerpendicularPlaneSegmentation(void)
{}

// output objects
std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr> PerpendicularPlaneSegmentation_inliers_cloudvector;
std::vector<std::vector<double>> PerpendicularPlaneSegmentation_coefficients_vectorvector;


void PerpendicularPlaneSegmentation::UsePerpendicularPlaneSegmentation(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_input, std::string  cloud_output_path)
{
	//////Perpendicular Plane Segmentation
	...
	while (cloud_input->points.size () > 0.3 * nr_points)
	{
		...
		PerpendicularPlaneSegmentation_coefficients_vectorvector.push_back(coefficients_vector);
		...
		PerpendicularPlaneSegmentation_inliers_cloudvector.push_back(inliers_cloud);
	...
	//Test Output for Plane Coefficients (it is a vector that contains one vector with coefficients per plane) 
	 std::cout << std::endl;
	 std::vector<double> coefficients_vector;
	 for (int i=0; i < PerpendicularPlaneSegmentation_coefficients_vectorvector.size(); i++){
	 	coefficients_vector = PerpendicularPlaneSegmentation_coefficients_vectorvector[i];
	...
	std::cout<< "Number of Planes	        (=Length Inliers_CloudVector): "<< PerpendicularPlaneSegmentation_inliers_cloudvector.size() <<std::endl;
	std::cout<< "Number of Coefficient Sets (=Length Coefficients_Vector): "<< PerpendicularPlaneSegmentation_coefficients_vectorvector.size() <<std::endl;
	std::cout << std::endl;
}


std::vector<std::vector<double>> PerpendicularPlaneSegmentation::getCoefficients_Vector(){
	return PerpendicularPlaneSegmentation_coefficients_vectorvector;
}
	
std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr> PerpendicularPlaneSegmentation::getInliers_CloudVector(){
	return PerpendicularPlaneSegmentation_inliers_cloudvector;
}
