...
// output objects
std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr> CylinderSegmentation_inliers_cloudvector;
std::vector<std::vector<double>> CylinderSegmentation_coefficients_vectorvector;
...
void CylinderSegmentation::UseCylinderSegmentation(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_input, std::string  cloud_output_path)
{
	...
	{
		// Segment the largest cylindric component from the remaining cloud
		seg.setInputCloud (cloud_input);
		seg.segment (*inliers_indices, *coefficients);
		if (inliers_indices->indices.size () == 0)
		{
			std::cerr << "Could not estimate a cylindric model for the given dataset." << std::endl;
			break;
		}
		...
		CylinderSegmentation_coefficients_vectorvector.push_back(coefficients_vector);
		...
		std::cerr << "PointCloud representing the cylindric component " << i << ": " << inliers_cloud->width * inliers_cloud->height << " data points." << std::endl;

		// write points to cloudvector to collect the pointclouds
		CylinderSegmentation_inliers_cloudvector.push_back(inliers_cloud);
	...
	//Test Output for Plane Coefficients (it is a vector that contains one vector with coefficients per plane) 
	 std::cout << std::endl;
	 std::vector<double> coefficients_vector;
	 for (int i=0; i < CylinderSegmentation_coefficients_vectorvector.size(); i++){
	 	coefficients_vector = CylinderSegmentation_coefficients_vectorvector[i];
	...
	std::cout<< "Number of Planes	        (=Length Inliers_CloudVector): "<< CylinderSegmentation_inliers_cloudvector.size() <<std::endl;
	std::cout<< "Number of Coefficient Sets (=Length Coefficients_Vector): "<< CylinderSegmentation_coefficients_vectorvector.size() <<std::endl;
	std::cout << std::endl;
}


std::vector<std::vector<double>> CylinderSegmentation::getCoefficients_Vector(){
	return CylinderSegmentation_coefficients_vectorvector;
}
	
std::vector<pcl::PointCloud<pcl::PointXYZRGBA>::Ptr> CylinderSegmentation::getInliers_CloudVector(){
	return CylinderSegmentation_inliers_cloudvector;
}
